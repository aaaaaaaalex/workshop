...
will break when it lands on a Calico Node which does not
recognize the connection.

Let's prepare to simulate a node churn event which will show this
condition happening.

Note that we should have a single persistent connection open on
our router instance for the duration of this page. If we lose that
connection for some reason, a new one must be opened, and we must
start these steps over from the top.
